...
class Wan22Trainer:
    def __init__(self, model, train_dataset, val_dataset=None, *, cfg: DictConfig):
        ...
        self.trace_alignment_resume = getattr(cfg, "trace_alignment_resume", None)
        ...
        trace_proj = getattr(self.model, "trace_proj", None)
        if trace_proj is not None:
            trainable_params.extend(list(trace_proj.parameters()))
        align_repr_proj = getattr(self.model, "align_repr_proj", None)
        if align_repr_proj is not None:
            trainable_params.extend(list(align_repr_proj.parameters()))
        ...
        self.checkpoint_root = getattr(self, 'checkpoint_save_path', self.checkpoint_root) 
    ...
    def _estimate_total_train_steps(self) -> int:
        ...
        # Total optimization steps for per epoch (e.g. 1000)
        # but here we have multiple micro_step_per_epoch
    ...
    def _resume_or_load_checkpoint(self):
        ...
        metadata_path = resume_path
        metadata_payload = payload
        if self.trace_alignment_resume:
            trace_resume_path = Path(str(self.trace_alignment_resume))
            if not trace_resume_path.exists():
                raise FileNotFoundError(f"Trace alignment checkpoint not found: {self.trace_alignment_resume}")
            logger.info("Loading trace alignment weights only: %s", trace_resume_path)
            metadata_payload = self.accelerator.unwrap_model(self.model).load_trace_alignment_checkpoint(str(trace_resume_path))
            metadata_path = trace_resume_path
        self._restore_weight_only_training_metadata(metadata_path, metadata_payload)
        logger.warning("Loaded weights and recoverable metadata only; optimizer state was not restored.")

    def _restore_weight_only_training_metadata(self, resume_path: Path, payload):
        if not isinstance(payload, dict):
            return

        step_dir = None
        if "module" in payload and resume_path.parent.name == "pytorch_model":
            step_dir = resume_path.parent.parent
        elif resume_path.name == "mp_rank_00_model_states.pt":
            step_dir = resume_path.parent.parent
        ...
        if step_dir is None or not step_dir.is_dir():
            if "step" in payload and payload["step"] is not None:
                ...
                logger.info("Restored global_step=%d from weight checkpoint payload.", self.global_step)
            return

        trainer_state_file = step_dir / "trainer_state.json"
        if trainer_state_file.exists():
            ...
            saved_batch_in_epoch = int(trainer_state.get("batch_in_epoch", 0))
            saved_num_processes = int(payload.get("dp_world_size", self.accelerator.num_processes))
            saved_sample_offset = saved_batch_in_epoch * self.batch_size * max(saved_num_processes, 1)
            current_global_batch = self.batch_size * max(int(self.accelerator.num_processes), 1)
            self.batch_in_epoch = saved_sample_offset // max(current_global_batch, 1)
            remainder = saved_sample_offset % max(current_global_batch, 1)

            self.train_sampler.set_epoch_offset(self.epoch)
            self.train_sampler.set_resume_batch_offset(self.batch_in_epoch)
            logger.info(
                "Restored weight-only trainer metadata: step=%d epoch=%d saved_batch_in_epoch=%d saved_world_size=%d current_batch_in_epoch=%d sample_offset=%d",
                self.global_step,
                self.epoch,
            ...
            if remainder != 0:
                logger.warning(
                    "Saved dataloader sample offset %d is not divisible by current global batch size %d; resumed offset is rounded down by %d samples.",
                    saved_sample_offset,
                    current_global_batch,
                    remainder,
                )

        scheduler_file = step_dir / "scheduler.bin"
        if scheduler_file.exists():
            scheduler_state = torch.load(scheduler_file, map_location="cpu")
            self.scheduler.load_state_dict(scheduler_state)
            last_lrs = scheduler_state.get("_last_lr")
            if isinstance(last_lrs, list) and len(last_lrs) == len(self.optimizer.param_groups):
                for param_group, lr in zip(self.optimizer.param_groups, last_lrs):
                    param_group["lr"] = float(lr)
            logger.info("Restored scheduler state from %s at last_epoch=%s.", scheduler_file, scheduler_state.get("last_epoch"))
    ...
    def _apply_dit_only_train_mode(model):
        ...
        traceanything = getattr(model, "traceanything", None)
        if traceanything is not None:
            traceanything.eval()
            traceanything.requires_grad_(False)
        trace_proj = getattr(model, "trace_proj", None)
        if trace_proj is not None:
            trace_proj.train()
            trace_proj.requires_grad_(True)
        align_repr_proj = getattr(model, "align_repr_proj", None)
        if align_repr_proj is not None:
            align_repr_proj.train()
            align_repr_proj.requires_grad_(True)
    ...
    def _to_batched_eval_sample(sample):
        ...
        trace_video = sample.get("trace_video", None)
        ...
        if trace_video is not None:
            if not isinstance(trace_video, torch.Tensor):
                raise TypeError(f"`sample['trace_video']` must be a torch.Tensor, got {type(trace_video)}")
            if trace_video.ndim == 5:
                trace_video = trace_video.unsqueeze(0)
            if trace_video.ndim != 6:
                raise ValueError(
                    "`sample['trace_video']` must be [B,num_cameras,T,3,H,W] "
                    f"or [num_cameras,T,3,H,W], got {tuple(trace_video.shape)}"
                )
    ...
    def _prune_old_checkpoints(self, keep: int = 2):
        if keep <= 0:
            return
        ...
        if weights_root.exists():
            for path in weights_root.glob("step_*.pt"):
                ...
                if match is not None and path.is_file():
                    checkpoint_steps.add(int(match.group(1)))
        ...
        keep_steps = set(sorted(checkpoint_steps, reverse=True)[:keep])

        if weights_root.exists():
            for path in weights_root.glob("step_*.pt"):
                match = step_pattern.match(path.stem)
                if match is not None and int(match.group(1)) not in keep_steps and path.is_file():
                    path.unlink()
        ...
        if state_root.exists():
            for path in state_root.glob("step_*"):
                match = step_pattern.match(path.name)
                if match is not None and int(match.group(1)) not in keep_steps and path.is_dir():
                    shutil.rmtree(path)
                    logger.info("[ckpt] pruned old training state checkpoint: %s", path)
    ...
    def save_checkpoint(self):
        ...
        if self.accelerator.is_main_process:
            self._prune_old_checkpoints(keep=2)
        self.accelerator.wait_for_everyone()
