...
def create_fastwam(
    ...
    if isinstance(traceanything, DictConfig):
        traceanything = OmegaConf.to_container(traceanything, resolve=True)
    if traceanything is not None and not isinstance(traceanything, dict):
        raise ValueError(f"`traceanything` must be dict-like, got {type(traceanything)}")
    ...
    return FastWAM.from_wan22_pretrained(
        ...
        traceanything_config=traceanything,
    )
