...
class FastWAMJoint(FastWAM):
    ...
    TraceAnything alignment is inherited from FastWAM.training_loss(). Because
    this class overrides _build_mot_attention_mask(), the inherited alignment
    runs under the joint action->full-video attention pattern.
    """
    ...
    def training_loss(self, sample, tiled: bool = False):
        """
        Train FastWAMJoint with the base FastWAM losses, including optional
        TraceAnything alignment.

        Important: this method intentionally delegates to ``FastWAM.training_loss``.
        The parent implementation calls ``self._build_mot_attention_mask(...)``
        at runtime, so this subclass still uses the joint mask defined above:

            action -> full video

        Therefore, when TraceAnything alignment is enabled through
        ``traceanything_config``, the aligned video hidden states can be attended
        by action tokens in the same forward pass. This is the key difference
        from the first-frame-only FastWAM mask.
        """
