...
class MoT(nn.Module):
    ...
    def forward(
        ...
        capture_layer_idx: Optional[int] = None,
        capture_experts: Sequence[str] = ("video",),
    ):
        ...
        if capture_layer_idx is not None and not (0 <= capture_layer_idx < self.num_layers):
            raise ValueError(
                f"`capture_layer_idx` must be in [0, {self.num_layers - 1}], got {capture_layer_idx}"
            )
        ...
        for layer_idx in range(self.num_layers):
            ...
            if capture_layer_idx is not None and layer_idx == capture_layer_idx:
                captured = {
                    name: tokens_all[name]
                    for name in capture_experts
                    if name in tokens_all
                }
