...
lingbot4dwam_train_cfg = EasyDict(__name__='Config: VA Lift2 test train')
lingbot4dwam_train_cfg.update(va_robotwin_cfg)

# Small Step3-compatible LeRobot test set (aligned with 4dwam_lift2_merged).
...
lingbot4dwam_train_cfg.dataset_path = os.path.join(
    ...
    'lift2_test_step3_compatible_60hz',
)

# Same action-norm pipeline as lift2_merged. If the test set has no stats file,
# reuse the merged-set stats so smoke runs stay consistent.
lingbot4dwam_train_cfg.action_norm_stats_path = os.path.join(
    lingbot4dwam_train_cfg.dataset_path,
    'meta',
    'action_norm_stats.json',
)
if not os.path.isfile(lingbot4dwam_train_cfg.action_norm_stats_path):
    lingbot4dwam_train_cfg.action_norm_stats_path = os.path.join(
        lingbot4dwam_train_cfg.data_path,
...
lingbot4dwam_train_cfg.keyword = (
    f'lift2_test_4DWAM_{lingbot4dwam_train_cfg.align_layer}'
)
