...
class TraceAnything(nn.Module):
    ...
    def _encode_extraction(self, views, chunk_size=400):
        # views[0]["img"]: (B, C, H, W)
        # B: batch size
        B = views[0].shape[0]
    ...
    @torch.no_grad()
    def extract_seq_features(self, views, timesteps):
        encoded_feats, positions, shapes = self._encode_extraction(views)
        ...
        for i, ef in enumerate(encoded_feats):
            ...
            image_times.extend([timesteps[i]] * num_patches)
        ...
        return rearrange(dec_output[-1], 'B (n P) D -> (n B) P D', n=num_images, P=P)
