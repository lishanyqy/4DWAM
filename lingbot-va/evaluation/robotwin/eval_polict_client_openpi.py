...
def main(usr_args):
    ...
    logger = Attender(
        index = f'{task_name}_tiny',
        filePath = f'/root/logs/robotwin/res/{task_name}_tiny.log',
    )
...
def eval_policy(
    ...
    args["eval_mode"] = True
    logger.info('We are now starting eval....')
    while succ_seed < test_num:
        ...
        print('Test Acc---------', TASK_ENV.suc / test_num)
        logger.info(f'Test Acc---------{TASK_ENV.suc / test_num}----------')
        now_id += 1
